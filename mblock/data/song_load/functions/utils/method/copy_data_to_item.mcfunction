summon item ~ ~ ~ {Item: {id: "enchanted_book", Count: 1b, tag: {contains_song: 1b, song: {xsize: -1, ysize: -1, zsize: -1, bpm: -1}, display: {Name: '"§aMy Song!"'}}}, Tags: [saved_song, _saved], PickupDelay: 0s}
...
execute store result entity @e[tag=saved_song,limit=1] Item.tag.song.zsize int 1 run scoreboard players get #song_length sload_tmp
