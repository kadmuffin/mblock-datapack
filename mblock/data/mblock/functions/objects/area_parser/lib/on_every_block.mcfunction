...
execute as @s[tag=!_summon_player,tag=on_bottom] if entity @p[gamemode=creative] if block ~ ~-1 ~ air if block ~ ~-2 ~ air if block ~ ~-3 ~ air run setblock ~ ~-1 ~ stripped_spruce_wood
