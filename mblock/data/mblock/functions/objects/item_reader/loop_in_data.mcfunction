...
execute as @s[tag=!counting_air] unless data entity @s Item.tag.z_data[0].sound_dir if score #sound_dir _mbtmp matches 0 if score #sound _mbtmp matches 2 if block ~ ~-2 ~ air run setblock ~ ~-2 ~ stripped_spruce_wood
...
execute as @s[tag=!counting_air,tag=!look_neg,scores={aparse.dir=0}] positioned ~ ~ ~1 run function mblock:objects/item_reader/events/load_events
execute as @s[tag=!counting_air,tag=!look_neg,scores={aparse.dir=1}] positioned ~1 ~ ~ run function mblock:objects/item_reader/events/load_events
