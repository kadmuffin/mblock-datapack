scoreboard players add interal_zsize _array_tmp 1
...
execute if block ~ ~ ~ magenta_glazed_terracotta run scoreboard players operation interal_zsize _array_tmp -= @s aparse.zsize
...
execute as @s[tag=!end_recursion] unless score #tone _mbtmp matches -1 if score #sound _mbtmp matches 1.. if score #sound_dir _mbtmp matches 2.. unless block ~ ~-1 ~ #mblock:replace_enabled_blocks unless block ~ ~1 ~ #mblock:replace_enabled_blocks store result entity @s Item.tag.tmp.sound_dir int 1 run scoreboard players get #sound_dir _mbtmp
