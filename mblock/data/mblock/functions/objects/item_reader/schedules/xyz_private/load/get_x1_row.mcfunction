...
execute if score @s aparse.xsize matches 2.. unless score #xsize aparse.xsize matches ..-1 if score #xsize aparse.xsize < @s aparse.xsize if entity @s[scores={aparse.dir=0}] positioned ~1 ~ ~ run function mblock:objects/item_reader/schedules/xyz_private/load/x_axis/horizontal_plus_x
