...
execute as @e[tag=_in_wait_for_load,limit=1] unless entity @e[tag=_load_scheduled] at @s align xyz positioned ~.5 ~.5 ~.5 positioned ~ ~1 ~ run function mblock:objects/item_reader/schedules/xyz_private/load/setup_data
execute as @e[tag=_in_wait_for_load_1,limit=1] unless entity @e[tag=_load_scheduled] at @s run function mblock:objects/item_reader/prepare_loop
