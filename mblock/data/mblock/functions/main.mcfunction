...
execute as @e[tag=_block] at @s positioned ~ ~ ~ unless entity @e[tag=player,distance=0..1.6] unless entity @e[nbt={OnGround:1b},tag=player,tag=extended,distance=0..2.6] positioned ~ ~-1 ~ unless entity @e[nbt={OnGround:1b},tag=player,tag=extended,distance=0..2.6] positioned ~ ~1 ~ run kill @s
